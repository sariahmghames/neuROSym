...
class Encoder(nn.Module):
    ...
    def init_hidden(self, batch):
        return (
            ...
            torch.zeros(self.num_layers, batch, self.h_dim).cuda()
        ) # num_layers for LSTM , return 2 initialisation because hidden for x entries and hidden for y entry
...
class Decoder(nn.Module):
    ...
    def __init__(
        self, seq_len, embedding_dim=64, h_dim=32, mlp_dim=1024, num_layers=1,
    ...
    def forward(self, last_pos, last_pos_rel, state_tuple, seq_start_end):
        ...
        for _ in range(self.seq_len):
            ...
            if self.pool_every_timestep: # every timestep predicted for the decoder output
                ...
                pool_h = self.pool_net(decoder_h, seq_start_end, curr_pos) # pool decoder hidden , as the encoder pooling mechanism 
                decoder_h = torch.cat(
...
class PoolHiddenNet(nn.Module): # pooling hidden state of all people present in scene. encoder output and decoder output
    ...
    def __init__(
        ...
        mlp_pre_dim = embedding_dim + h_dim # 1st mlp in fig 3 (pooling module)
        mlp_pre_pool_dims = [mlp_pre_dim, 512, bottleneck_dim] # 2nd mlp in pooling module, mlp_pre_dim is the input size to mlp, mlp hidden layer size = 512, output size = bottleneck_dim
    ...
    def forward(self, h_states, seq_start_end, end_pos):
        ...
        for _, (start, end) in enumerate(seq_start_end): # start and end are indices
            ...
            curr_hidden = h_states.view(-1, self.h_dim)[start:end] # in the current considered sequence , 
            #if (start==0):
            #print("curr_hidden shape=", curr_hidden.shape) # shape= torch.Size([2, 32]), torch.Size([4, 32]), torch.Size([5, 32]), torch.Size([4, 32]) ... where 2, 4 .. are num_ped
            curr_end_pos = end_pos[start:end]

            #if (start==0):
            #    print("curr_end_pos shape=", curr_end_pos.shape) # torch.Size([2, 2])

            # Repeat -> H1, H2, "H1, H2"
            curr_hidden_1 = curr_hidden.repeat(num_ped, 1) # 1 is for axis

            #if (start==0):
            #print("curr_hidden_1 shape=", curr_hidden_1.shape) #  torch.Size([4, 32])

            # Repeat position -> P1, P2, "P1, P2"
            curr_end_pos_1 = curr_end_pos.repeat(num_ped, 1) 
            #if (start==0):
            #    print("curr_end_pos_1 shape=", curr_end_pos_1.shape) # torch.Size([4, 2])
            # Repeat position -> P1, P1, P2, P2
            curr_end_pos_2 = self.repeat(curr_end_pos, num_ped)
            #if (start==0):
            #    print("curr_end_pos_2 shape=", curr_end_pos_2.shape) # torch.Size([4, 2])
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        self, obs_len, pred_len, embedding_dim=64, encoder_h_dim=32,
        decoder_h_dim=32, mlp_dim=1024, num_layers=1, noise_dim=(0, ),
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, user_noise=None):
        ...
        if self.pooling_type:
            end_pos = obs_traj[-1, :, :]
            pool_h = self.pool_net(final_encoder_h, seq_start_end, end_pos) # pool hidden state of encoder
        ...
        if self.mlp_decoder_needed():
            noise_input = self.mlp_decoder_context(mlp_decoder_context_input) # mlp_decoder_context_input is the input to mlp_decoder_context
        else:
